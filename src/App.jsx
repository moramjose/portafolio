        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="text-center md:text-left">
              <a href="#" className="text-primary font-mono font-medium text-lg">{'<JM />'}</a>
              <p className="text-text-muted text-sm mt-1">{t.footer.builtWith}</p>
            </div>
            ...
            <div className="flex gap-6">
              {navLinks.map((link) => (
                <a
                  key={link.href}
                  href={link.href}
                  className="text-text-muted hover:text-primary transition-colors text-sm hidden md:block"
                >
                  {t.nav[link.label]}
                </a>
              ))}
          ...
          <div className="text-center mt-8 pt-6 border-t border-border">
            <p className="text-text-muted text-sm font-mono">&copy; {new Date().getFullYear()} {t.footer.copyright}</p>
          </div>
        </div>
      </footer>
    </div>
